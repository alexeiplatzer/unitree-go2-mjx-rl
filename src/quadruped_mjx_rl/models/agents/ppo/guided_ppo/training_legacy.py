...
from . import losses as teacher_losses
...
def train(
    ...
    obs_shape = jax.tree_util.tree_map(lambda x: x.shape[2:], env_state.obs["state"])
    ...
    loss_fn = functools.partial(
        teacher_losses.compute_teacher_loss,
    ...
    def minibatch_step(
        ...
        _, student_params, student_optimizer_state = student_gradient_update_fn(
            student_params,
            teacher_params,
            normalizer_params,
            data,
    ...
    def sgd_step(
        ...
        optimizer_state, teacher_params, student_optimizer_state, student_params, key = carry
    ...
    init_params = teacher_losses.TeacherNetworkParams(
    ...
    if (  # TODO: add student network here
        restore_checkpoint_path is not None and epath.Path(restore_checkpoint_path).exists()
    ):
    ...
    for it in range(num_evals_after_init):
        ...
        if process_id == 0:
            ...
            params = _unpmap((training_state.normalizer_params, training_state.teacher_params))
