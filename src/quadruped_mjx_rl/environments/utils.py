...
class Base:
    ...
    def index_set(
        self, idx: jax.Array | Sequence[jax.Array], o: Any
    ) -> Any:
...
class Inertia(Base):
    ...
    def mul(self, m: Motion) -> 'Force':
        ...
        ang =     jnp.dot(self.i, m.ang) +     jnp.cross(self.transform.pos, m.vel)
        vel = self.mass * m.vel -     jnp.cross(self.transform.pos, m.ang)
...
class System(mjx.Model):
    ...
    init_q: jax.Array
    # non-pytree nodes
    enable_fluid: bool = flax_struct.field(pytree_node=False)
...
def _(m: Motion, self: Motion) -> Motion:
    vel = jnp.cross(self.ang, m.vel) + jnp.cross(self.vel, m.ang)
