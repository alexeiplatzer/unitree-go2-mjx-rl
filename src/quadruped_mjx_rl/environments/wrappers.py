...
def wrap_for_training(
    ...
    Args:
        ...
        randomization_config: a randomization config that describes a vectorized system
    ...
    """
    if randomization_config is None and not vision:
        env = VmapWrapper(env)
    else:
...
def _vmap_wrap_with_randomization(
    ...
    if vision:
        if not randomization_config:
            randomization_fn = _identity_vision_randomization_fn
        else:
            randomization_fn = functools.partial(
                _supplement_vision_randomization_fn,
                randomization_fn=randomization_config.domain_randomize,
            )
    else:
        randomization_fn = randomization_config.domain_randomize

    # TODO: refactor the randomization wrappers to work with rand configs instead of fns
    wrapped_env = wrapper(
        env=env,
...
class DomainRandomizationVmapWrapper(Wrapper):
    ...
    def get_vision_obs(
        ...
        return jax.vmap(get_vision_obs, in_axes=[self._in_axes, 0, 0])(
            self._sys_v,
            pipeline_state,
            state_info,
        )
...
def _supplement_vision_randomization_fn(
    ...
    for field in required_fields:
        if getattr(in_axes.model, field) is None:
            ...
            val = (
                jnp.repeat(-1, pipeline_model.model.geom_matid.shape[0], 0)
                if field == "geom_matid"
                else getattr(pipeline_model.model, field)
            )
            pipeline_model = pipeline_model.replace(
                model=pipeline_model.model.tree_replace(
                    {
