...
class JoystickEnv(PipelineEnv):
    ...
    def __init__(
        ...
        self._vision = self.vision_config is not None
        if self._vision:
            from madrona_mjx.renderer import BatchRenderer
            self.renderer = BatchRenderer(
                m=self._mjx_model,
                gpu_id=self.vision_config.gpu_id,
                num_worlds=self.vision_config.render_batch_size,
                batch_render_view_width=self.vision_config.render_width,
                batch_render_view_height=self.vision_config.render_height,
                enabled_geom_groups=np.asarray(self.vision_config.enabled_geom_groups),
                enabled_cameras=np.asarray([0,]),
                add_cam_debug_geo=False,
                use_rasterizer=self.vision_config.use_rasterizer,
                viz_gpu_hdls=None,
            )
    ...
    def reset(self, rng: jax.Array) -> State:  # pytype: disable=signature-mismatch
        ...
        if self._vision:
            ...
            state_info.update({"obs_history": obs_history})
